...
public:
  ...
  matchAndRewrite(mlir::stablehlo::ClampOp srcOp,
    ...
    LogicalResult legalityResult =
        ttmlir::utils::broadcastValue(rewriter, adaptor.getMin(), outputType,
                                      minTensor, srcOp->getLoc(), false);
    if (!legalityResult.succeeded()) {
      return rewriter.notifyMatchFailure(
    ...
    legalityResult =
        ttmlir::utils::broadcastValue(rewriter, adaptor.getMax(), outputType,
                                      maxTensor, srcOp->getLoc(), false);
    if (!legalityResult.succeeded()) {
      return rewriter.notifyMatchFailure(
