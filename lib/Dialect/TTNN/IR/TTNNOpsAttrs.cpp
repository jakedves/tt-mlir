...
bool CoreRangeAttr::intersects(const CoreRangeAttr &other) const {
...
::llvm::LogicalResult CoreRangeSetAttr::verify(
  ...
  for (auto outerIt = coreRanges.begin(); outerIt != coreRanges.end() - 1;
       outerIt++) {
    for (auto innerIt = outerIt + 1; innerIt != coreRanges.end(); innerIt++) {
      ...
      if (firstCoreRange.intersects(secondCoreRange)) {
        emitError() << "Core ranges overlap: " << firstCoreRange << " and "
                    << secondCoreRange;
        return ::llvm::failure();
      }
    }
